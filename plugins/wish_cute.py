...
SUPPORT_CHAT = "+cXIPgHSuJnxiNjU1"

@app.on_message(filters.command(["wish", "ish"], prefixes=["/", ".", "!", "W", "w"]))
async def wish(_, m):
    if len(m.command) < 2:
...
@app.on_message(filters.command(["cute", "ute"], prefixes=["/", "!", ".", "C", "c"]))
